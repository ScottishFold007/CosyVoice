...
class Upsample1D(TransformerEngineBaseModule):  
    ...
    def forward(self, inputs: torch.Tensor, input_lengths: torch.Tensor, conv_cache: torch.Tensor = torch.zeros(0, 0, 0)) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:  
        # 使用TransformerEngine的prepare_forward进行FP8相关准备  
        with self.prepare_forward(inputs, num_gemms=self.num_gemms):  
...
class PreLookaheadLayer(TransformerEngineBaseModule):  
...
class UpsampleConformerEncoder(TransformerEngineBaseModule):  
    ...
    def __init__(  
        ...
        # 计算GEMM操作数量  
        # 每个Conformer编码器层的GEMM操作数量  
        gemms_per_encoder_layer = 2  # 基础：自注意力1个，前馈网络1个  
        if macaron_style:  
            gemms_per_encoder_layer += 1  # 额外的前馈网络  
        if use_cnn_module:  
            gemms_per_encoder_layer += 1  # 卷积模块  
              
        # 总GEMM操作数量 = 子采样层 + 预处理层 + 编码器层 + 上采样层 + 上采样后的子采样层 + 上采样后的编码器层  
        self.num_gemms = (  
            1 +  # 子采样层  
            self.pre_lookahead_layer.num_gemms +  # 预处理层  
            num_blocks * gemms_per_encoder_layer +  # 编码器层  
            self.up_layer.num_gemms +  # 上采样层  
            1 +  # 上采样后的子采样层  
            4 * gemms_per_encoder_layer  # 上采样后的编码器层  
        )  
    ...
    def forward(  
        ...
        """  
        # 使用TransformerEngine的prepare_forward进行FP8相关准备  
        with self.prepare_forward(xs, num_gemms=self.num_gemms):  
            T = xs.size(1)  
    ...
    def forward_chunk(  
        ...
        Args:  
            xs: chunk input tensor (B, T, D)  
            xs_lens: input length (B)  
            offset: current offset in encoder output time stamp  
            context: context tensor for lookahead  
            pre_lookahead_layer_conv2_cache: cache for pre-lookahead layer  
            encoders_kv_cache: cache for encoder key-value  
            upsample_offset: offset for upsampled sequence  
            upsample_conv_cache: cache for upsampling convolution  
            upsample_kv_cache: cache for upsampled encoder key-value  
        Returns:  
            encoder output tensor xs, masks, and caches  
        """  
        # 使用TransformerEngine的prepare_forward进行FP8相关准备  
        # 推理时每个chunk的GEMM操作数量较少  
        with self.prepare_forward(xs, num_gemms=5):  # 使用较小的GEMM数量以提高效率  
            assert xs.size(0) == 1  
